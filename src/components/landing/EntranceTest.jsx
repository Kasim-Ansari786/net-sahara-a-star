...
const EXAM_DATE = new Date("2025-05-17T10:00:00+05:30");
...
const Cell = ({ value, label }) => (
  <div className="bg-navy-deep/60 border border-gold/30 rounded-lg p-3 sm:p-4 backdrop-blur min-w-[68px] sm:min-w-[88px]">
    ...
    <div className="text-[10px] sm:text-xs uppercase tracking-[0.2em] text-ivory/60 mt-1">{label}</div>
  </div>
);

const details = [
  { icon: Calendar, label: "Date", value: "17 May 2025" },
...
const EntranceTest = () => {
  ...
  return (
    <section id="exam" className="py-20 sm:py-28 bg-hero-gradient text-ivory relative overflow-hidden">
      ...
      <div className="relative container">
        <div className="max-w-3xl mx-auto text-center mb-12">
          <div className="text-xs uppercase tracking-[0.3em] text-gold font-medium mb-4">National Entrance Test</div>
          <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl mb-5">
            Gateway to <em className="not-italic text-gradient-gold">A-Star Academy</em>
          </h2>
          <p className="text-ivory/75 text-lg">Secure your seat in India's most aspirational hospitality program.</p>
        </div>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-2 sm:grid-cols-5 gap-3 mb-10">
          {details.map((d) => (
            <div key={d.label} className="bg-navy-deep/40 border border-gold/20 rounded-lg p-4 text-center backdrop-blur">
              <d.icon className="h-5 w-5 text-gold mx-auto mb-2" />
              <div className="text-[10px] uppercase tracking-[0.15em] text-ivory/60">{d.label}</div>
              <div className="font-display text-base sm:text-lg text-ivory mt-1">{d.value}</div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-ivory/60 mt-4 uppercase tracking-[0.2em]">Limited seats · First come, first served</p>
        </div>
      </div>
    </section>
  );
